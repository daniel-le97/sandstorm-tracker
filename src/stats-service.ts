...
export class StatsService {
        ...
        try {
            switch (event.type) {
                ...
                case "suicide":
                    ...
                    break;
                case "chat_command":
                    this.handleChatCommand(event as ChatEvent, serverId);
                    break;
                case "round_over":
     ...
     */
    private static handleChatCommand(event: ChatEvent, serverId: number): void {
        const timestamp = new Date(this.parseTimestamp(event.timestamp));

        // Get or create player
        const player = getStatements().upsertPlayer.get(event.data.steamId, event.data.playerName, serverId) as any;
     ...
     */
    static endMatch(serverId: number, status: "completed" | "aborted" = "completed"): void {
        const serverKey = serverId.toString();
        const activeMatch = activeMatches.get(serverKey);
        ...
        console.log(
            `${status === "completed" ? "Completed" : "Aborted"} match ${
                activeMatch.matchId
