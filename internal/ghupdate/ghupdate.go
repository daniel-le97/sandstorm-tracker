...
// Plugin defines the public interface for the ghupdate plugin.
type Plugin interface {
	// Update performs the update with optional backup.
	Update(withBackup bool) error
	// CheckForUpdate checks if a newer version is available without updating.
...
func Register(app core.App, rootCmd *cobra.Command, config Config) (Plugin, error) {
	...
	app.Store().Set("plugin:ghupdate", p)

	return p, nil
}

// GetPlugin retrieves the ghupdate plugin from the app store with proper type assertion.
func GetPlugin(app core.App) (Plugin, error) {
	p := app.Store().Get("plugin:ghupdate")
	if p == nil {
		return nil, errors.New("ghupdate plugin not found in store")
	}

	plugin, ok := p.(*plugin)
	if !ok {
		return nil, fmt.Errorf("stored value is not a ghupdate plugin, got %T", p)
	}
...
// Update performs the update with optional backup (implements Plugin interface).
func (p *plugin) Update(withBackup bool) error {
	return p.update(withBackup)
}

// CheckForUpdate checks if a newer version is available without updating (implements Plugin interface).
func (p *plugin) CheckForUpdate(ctx context.Context) (string, error) {
...
func (p *plugin) update(withBackup bool) error {
	...
	p.app.Logger().Info("Updated to version " + latest.Tag)
	return nil
}
