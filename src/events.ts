...
export function parseLogEvent ( logLine: string ): GameEvent | null {
    ...
    const killMatch = trimmedLine.match( /LogGameplayEvents: Display: (.+?)\[(\d+|INVALID), team (\d+)\] killed (.+?)\[(\d+|INVALID), team (\d+)\] with (.+)$/ );
    if ( killMatch && killMatch[ 1 ] && killMatch[ 2 ] && killMatch[ 3 ] && killMatch[ 4 ] && killMatch[ 5 ] && killMatch[ 6 ] && killMatch[ 7 ] )
    {
    ...
    // Round Over
    const roundOverMatch = trimmedLine.match( /LogGameplayEvents: Display: Round (\d+) Over: Team (\d+) won \(win reason: (.+)\)$/ );
    ...
    // Map Load
    const mapLoadMatch = trimmedLine.match( /LogLoad: LoadMap: \/Game\/Maps\/(.+?)\/(.+?)\?.*?Scenario=Scenario_(.+?)_(.+?)_(.+?)\?.*?MaxPlayers=(\d+)\?.*?Lighting=(.+?)(?:\?|$)/ );
    if ( mapLoadMatch && mapLoadMatch[ 1 ] && mapLoadMatch[ 3 ] && mapLoadMatch[ 4 ] && mapLoadMatch[ 5 ] && mapLoadMatch[ 6 ] && mapLoadMatch[ 7 ] )
    {
    ...
    if ( trimmedLine.includes( 'LogMapVoteManager: Display: Existing Vote Options:' ) ||
